...
int items = 0;
uthread_sem_t mutex;
uthread_sem_t needItem;
uthread_sem_t haveItem;
...
void* consumer (void* v) {
    for (int i=0; i<NUM_ITERATIONS; i++) {
        uthread_sem_wait(haveItem);
        uthread_sem_wait(mutex);
        items -= 1;
        assert (items >= 0 && items <= MAX_ITEMS);
        uthread_sem_signal(mutex);
        uthread_sem_signal(needItem);
        histogram [items] ++;
    }
    return NULL;
}
...
int main (int argc, char** argv) {
    ...
    needItem = uthread_sem_create(MAX_ITEMS);
    haveItem = uthread_sem_create(0);
